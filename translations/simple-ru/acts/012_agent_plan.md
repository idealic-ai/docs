...
- **Направленный:** Связи идут только в одну сторону. Шаг, который создаёт какие-то данные, всегда должен идти *перед* шагом, который эти данные использует.
...
Самая сильная сторона этой системы — полное разделение планирования и выполнения. Поскольку `План` — это просто описание действий (данные), а не сама программа, ИИ может составить всю схему `Вызовов Инструментов` *до того*, как начнётся какая-либо работа.
