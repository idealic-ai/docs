...
Сочетание записи в состояние через `_outputPath` и чтения из него с помощью **ссылок на переменные** является ключевым механизмом, который позволяет отделить планирование от выполнения. Это дает агенту возможность построить полный граф потока данных — цепочку `Tool Calls`, связанных ссылками — *до* запуска любого инструмента.
