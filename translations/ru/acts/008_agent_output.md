...
> Мета-свойство `_outputPath` в :term[Вызове]{canonical="Call"}, которое указывает, куда сохранять результат работы инструмента. Это строка, которая сообщает движку исполнения, куда поместить результат, делая его доступным для последующих шагов.
...
> Sidenote:
> Хотя целью может быть любое :term[Сообщение с Данными]{canonical="Data Message"}, наиболее частым случаем является запись в :term[Сообщение о Состоянии]{canonical="State Message" href="./009_agent_state.md"} для сохранения информации на протяжении нескольких шагов процесса.
...
- **`_date`**: ISO-метка времени, когда был записан вывод.

Это обеспечивает полный, проверяемый след изменений контекста, что неоценимо для отладки и отслеживания логики агента.
...
```json
// Схема инструмента фиксирует _outputPath на определенном значении
{
  "_outputPath": {
    "type": "string",
...
Когда необходимо разрешить :term[Ссылку на Переменную]{canonical="Variable Reference" href="./007_agent_variables.md"}, например `†data.user.name`, движок ищет в сообщениях контекста в **обратном хронологическом порядке**. Он проверяет каждое сообщение от нового к старому, и первое, которое содержит запрашиваемый путь (`user.name`), предоставляет значение. Эта стратегия «последняя запись побеждает» гарантирует, что всегда используется самое последнее значение, без сложной логики объединения.
...
Сила сочетания :term[Ссылок на Переменные]{canonical="Variable Reference"} с :term[Путями Вывода]{canonical="Output Path"} заключается в их способности определять операции над данными, которые еще не доступны. Например, :term[Вызов Инструмента]{canonical="Call"} может быть определен для работы со значением из :term[Входного]{canonical="Input"} сообщения, даже если этот конкретный ввод еще не был предоставлен. Это позволяет создавать многократно используемые, параметризованные рабочие процессы.

Эта концепция распространяется на объединение :term[Вызовов Инструментов]{canonical="Call"} в цепочки. :term[Вызов Инструмента]{canonical="Call"} может быть создан с :term[Ссылкой на Переменную]{canonical="Variable Reference"}, указывающей на :term[Путь Вывода]{canonical="Output Path"} _предыдущего_ вызова в той же последовательности. Это создает многошаговый поток данных, где вывод одного инструмента становится вводом для следующего.

## Взаимодействие с другими системами

- **:term[Сообщение с Данными]{canonical="Data Message"}:** `_outputPath` является основным механизмом для создания и обновления :term[Сообщений с Данными]{canonical="Data Message"} в рамках рабочего процесса. Он преобразует :term[Вызов Инструмента]{canonical="Tool Call"} без состояния в операцию с состоянием, сохраняя ее результат в контексте и делая его доступным для последующих шагов.
