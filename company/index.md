# Company Documentation

---

These documents articulate the operational core of our organization. They define the standards for communication, the protocols for execution, and the shared reality we commit to upholding. They are the source of truth for _how_ we work together.

> Sidenote:
> The company is a vehicle for the vision.
>
> - _Why?_ [Manifesto of Co-Liberation](../manifesto/)
> - _What?_ [Edict of Autonomy](../edict/)
> - _How?_ **Company Documentation**

This index bridges the gap between our abstract values and our daily actions. It serves as the manual for our collaboration, ensuring that every team member understands not just what we are building, but how we are building it.

---

### Theory

> Sidenote: The foundational pillars of our operational philosophy.

- [01: Pitch](./01_pitch.md)
- [02: Process](./02_process.md)

### Artifacts

> Sidenote: The definitions of the documents we produce.

- [20: Specification](./20_document_spec.md)
- [21: Proposal](./21_document_proposal.md)
- [22: Alignment](./22_document_alignment.md)

### Prompts

> Sidenote: Instructions for AI agents.

- [50: Prompt/Truth](./50_prompt_truth.md)
- [51: Prompt/Alignment](./51_prompt_alignment.md)
- [52: Prompt/Proposal](./52_prompt_proposal.md)
- [60: Prompt/Recipes](./60_prompt_recipes.md)
